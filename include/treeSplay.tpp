...
template <class T> bool TreeSplay<T>::insert(T key) {
	if(this->root_==NULL) {
		...
		if(scoreCompare == 0) {
			...
			this->root_->leftChild->rightChild = NULL;
		} else {
			this->root_ = new Node<T>(key, this->root_->leftChild, this->root_);
			this->root_->rightChild->leftChild = NULL;
		}
...
template <class T> bool TreeSplay<T>::find(T &key) {
...
template <class T> bool TreeSplay<T>::remove(T &key) {
	Node<T>* ptr = findNode(key);
	if(ptr==NULL||compare(key, ptr->key)!=0) return false;
	remove(ptr);
	key = ptr->key;
	delete ptr;
	return true;
